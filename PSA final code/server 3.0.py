...
class Dispute(db.Model):
    ...
    date_completed = db.Column(db.DateTime, nullable=True)
    status = db.Column(db.String(50), nullable=False)
    dispute_plaintiff = db.Column(
        db.Integer, db.ForeignKey('coor_user.keyID'), nullable=False)
    involved_users = db.Column(db.String(200))  # Comma-separated COOR IDs
    description = db.Column(db.Text, nullable=False)
    container_id = db.Column(db.Integer, db.ForeignKey(
        'container.containerID'), nullable=True)  # Add this column
    cargoID = db.Column(db.Integer, primary_key=True, nullable=True)
    coor_user_id = db.Column(db.Integer, db.ForeignKey(
        'coor_user.keyID'), nullable=True)
